...
class CitasScreen(Screen):
    ...
    def cargar_citas(self, fecha=None):
        ...
        try:
            ...
            if fecha:
                query = citas_ref.where(filter=firestore.FieldFilter("fecha", "==", fecha)).order_by("hora").stream()
            else:
                query = citas_ref.order_by("fecha").order_by("hora").stream()
            
            for doc in query:
                cita_data = doc.to_dict()
                monto=cita_data.get('monto', 0)
                btn = Button(
                    text=f"{cita_data.get('fecha', '')} {cita_data.get('hora', '')} - {cita_data.get('paciente_nombre', '')}\n"
                    f"Servicio: {cita_data.get('servicio', 'Consulta')}-${monto:.2f}",
                    size_hint_y=None,
                    height=120,
    ...
    def cargar_recordatorios(self):
        ...
        try:
            ...
            citas_ref = db.collection("citas")
            query = citas_ref.where(filter=firestore.FieldFilter("fecha", "in", [fecha.strftime("%d-%m-%Y") for fecha in fechas])).where(filter=firestore.FieldFilter("estado", "==", "pendiente"))
            
            for doc in query.stream():
                cita = doc.to_dict()
                monto=cita.get('monto', 0)
                btn = Button(
                    text=f"{cita['fecha']} {cita['hora']}\n"
                         f"{cita['paciente_nombre']}\n"
                         f"servicio: {cita.get('servicio', 'Consulta')}\n"
                         f"Monto: ${monto:.2f}",
                    size_hint_y=None,
                    height=140,
    ...
    def cargar_cumpleanos(self):
        try:
            ...
            for paciente in pacientes_ref.stream():
                ...
                try:
                    if 'fecha_nacimiento' not in paciente_data:
                        continue
                    ...
                    for dia in semana:
                        if fecha_nac.day == dia.day and fecha_nac.month == dia.month:
                            ...
                            lbl = Label(
                                text=f"{paciente_data.get('nombre', '')} {paciente_data.get('apellido', '')}\n" +
    ...
    def mostrar_detalle_cita(self, instance):
        ...
        Servicio: {cita_data.get('Servicio', 'Consulta')}
        Monto: ${monto:.2f}
    ...
    def modificar_cita(self, cita_id, cita_data):
        ...
        popup = Popup(title="Modificar Cita", size_hint=(0.9, 0.9))
        ...
        servicio_layout = BoxLayout(size_hint_y=None, height=50)
        servicio_layout.add_widget(Label(text="Servicio actual:"))
        servicio_actual = Label(text=cita_data.get('servicio', 'Consulta'))
        servicio_layout.add_widget(servicio_actual)
        layout.add_widget(servicio_layout)
    
        self.nuevo_servicio = TextInput(
            hint_text="Nuevo servicio",
            text=cita_data.get('Servicio', 'Consulta')
        )
        layout.add_widget(self.nuevo_servicio)
        
        monto_layout = BoxLayout(size_hint_y=None, height=50)
        monto_layout.add_widget(Label(text="Monto actual:"))
        monto_actual = Label(text=f"${cita_data.get('monto', 0):.2f}")
        monto_layout.add_widget(monto_actual)
        layout.add_widget(monto_layout)

        self.nuevo_monto = TextInput(
            hint_text="Nuevo monto",
            text=str(cita_data.get('monto', 0)),
            input_filter='float'
        )
        layout.add_widget(self.nuevo_monto)

        motivo_layout = BoxLayout(size_hint_y=None, height=50)
        motivo_layout.add_widget(Label(text="Motivo actual:"))
        motivo_actual = Label(text=cita_data.get('motivo', ''))
        motivo_layout.add_widget(motivo_actual)
        layout.add_widget(motivo_layout)

        self.nuevo_motivo = TextInput(
            hint_text="Nuevo motivo",
            text=cita_data.get('motivo', ''),
            multiline=True
        )
    ...
    def guardar_cambios_cita(self, cita_id):
        ...
        nuevo_servicio = self.nuevo_servicio.text.strip()
        nuevo_monto_str=self.nuevo_monto.text.strip()
        ...
        try:
            ...
            errors.append("Monto debe ser un número válido")
            
        if not nuevo_servicio:
            errors.append("Debe ingresar un servicio")
